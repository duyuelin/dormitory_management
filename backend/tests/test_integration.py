  ...
  class IntegrationTestCase(unittest.TestCase):
      ...
      def test_full_checkout_workflow(self):
          ...
          headers = {'Authorization': f'Bearer {self.admin_token}'}
          with self.app.app_context():
              Building.__table__.create(db.engine, checkfirst=True)
              Room.__table__.create(db.engine, checkfirst=True)
              Student.__table__.create(db.engine, checkfirst=True)
              CheckInRecord.__table__.create(db.engine, checkfirst=True)
      ...
      def test_repair_lifecycle(self):
          ...
          with self.app.app_context():
              Building.__table__.create(db.engine, checkfirst=True)
              Room.__table__.create(db.engine, checkfirst=True)
              RepairRecord.__table__.create(db.engine, checkfirst=True)
              b = Building(name='1号楼', code='B001')
              db.session.add(b)
              db.session.flush()
              r = Room(building_id=b.id, room_number='101', total_beds=4, available_beds=4)
